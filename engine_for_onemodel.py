...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module,criterion2,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #print("target after mixup", targets)
        if loss_scaler is None:
...
def train_one_epoch(args, model: torch.nn.Module, criterion: torch.nn.Module, criterion2,
                    ...
                    model_ema: Optional[ModelEma] = None, mixup_fn: Optional[Mixup] = None, log_writer=None,
                    start_steps=None, lr_schedule_values=None, wd_schedule_values=None,
                    num_training_steps_per_epoch=None, update_freq=None, buffer=None):

    model.train(True)
    metric_logger = utils.MetricLogger(delimiter="  ")
    print("Training with Disk-Based DER (8 samples per iteration)")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('min_lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = f'Epoch: [{epoch}]'
    print_freq = 1
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if mixup_fn is not None:
            print(f"Actual batch size received by mixup_fn: {len(samples)}")
            samples, targets = mixup_fn(samples, targets)
            if torch.isnan(targets).any():  # ✅ Check if Mixup causes NaN
                print("Warning: NaN detected in targets after Mixup, skipping batch.")
                continue

        # 🔹 Load only 8 samples & features from the oldest stored batch
        if data_iter_step>1:
            buf_samples, buf_features = buffer.load_data(device=device)
            print("buff_samples:", buff_samples, buff_features, end="/n")
            
        else:
            print("none---------------------------------------------none")
            buf_samples,buf_features= None, None

        # 🔹 Compute loss using both fresh & replayed samples
        if loss_scaler is None:
            samples = samples.half()
            loss, output, features = cross_train_class_batch(
                model, samples, targets, criterion, criterion2, buf_samples, buf_features
            )
        else:
            with torch.cuda.amp.autocast():
                samples = samples.half()
        ...
        if loss_scaler is None:
            ...
            if (data_iter_step + 1) % update_freq == 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # ✅ Prevents large gradient updates
        ...
        # 🔹 Save current batch to disk for replay
        buffer.save_data(data_iter_step, samples, features.data)
...
def final_test(args, data_loader, model, device, file):
    ...
    #print(f"Shapes: Output: {len(Out)}, labels:{len(labels)}, features: {features.shape}, {type(features)}") 
    #print("labels",labels)
