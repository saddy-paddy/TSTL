...
class QuickGELU(nn.Module):
    def forward(self, x: torch.Tensor):
        ...
        x= x * torch.sigmoid(1.702 * x) ## SIddhi added this, it was not present in original, just the commented out return statement.
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = torch.nn.functional.softmax(attn - attn.max(dim=-1, keepdim=True).values, dim=-1)
...
class Block(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        #self.attn_mask=None ### Sid added
        #print("This executed")
        if torch.isnan(x).any() or torch.isinf(x).any():
            print("X is nan or inf. Exit...")
            exit()
...
class STCrossTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            if s_x.isnan().any() or t_x.isnan().any():
                print("faulty 3")
            s_x, t_x = blk(s_x, t_x)
        s_x = s_x.permute(1,0,2)
    ...
    def forward(self, x):
        if self.composition:
            ...
            if s_x.isnan().any() or t_x.isnan().any():
                print("Faulty")
            #print("s_x:", s_x.shape, "t_x", t_x.shape)
...
@register_model
def bidir_vit_base_patch16_224(pretrained=True, **kwargs):
    model = STCrossTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, 
        norm_layer=partial(nn.LayerNorm, eps=1e-6), composition=False, **kwargs) ##siddhi removed **kwargs from the parameters
    return model
